...
class DeepSpeechService extends Client {
	...
	streamEnd() {
		...
		if (this.lastSilenceChunk) {
			...
			this.lastSilenceChunk = null;
			// this.log('check this.silenceBuffers', this.silenceBuffers);
		}
	}
	...
	processRecognition(text, stats) {
		if (this.state.hotword) {
			// if (
			// 	stats.hotword && /^[a-z]+$/.test(text)  || // any single word returned from deepspeech is assumed to be the hotword
			// 	text === this.state.hotword ||
			// 	text === 'bumble bee'  // need a list of hotword alternates
			// ) {
			// 	this.log('HOTWORD WAS recognized');
			// }
			// else {
			...
			if (text === 'he') return; // bug in DeepSpeech 0.7.3 emits "he" a lot
			...
			// 		this.log('HOTWORD EXACT');
			// 		return;
			// 	}
			// 	if (text.startsWith(hotwordSubstitutions[i])) {
			// 		this.log('HOTWORD REPLACE', hotwordSubstitutions[i]);
